...
func (h *authServiceServer) GuestAuth(ctx context.Context, _ *emptypb.Empty) (*authv1.GuestAuthResponse, error) {
	accessToken, guestID, tokenType, expiresIn, err := h.service.GuestAuth(ctx)
	if err != nil {
		return nil, h.handleError(err)
	}

	return &authv1.GuestAuthResponse{
		AccessToken: accessToken,
		GuestId:     guestID,
		TokenType:   tokenType,
		ExpiresIn:   int32(expiresIn),
