...
func (s *UserService) CreateUser(ctx context.Context, request *pb.CreateUserRequest) (*pb.User, error) {
	...
	createdUser, err := s.service.Create(ctx, request.User.Username, request.User.Password)
	if err != nil {
...
func (s *UserService) UpdateUser(ctx context.Context, request *pb.UpdateUserRequest) (*pb.User, error) {
	...
	updatedUser, err := s.service.Update(ctx, &user.UpdateParams{
		ID:              userID,
		User:            updateModel,
		CurrentPassword: request.CurrentPassword,
		UpdateMask:      request.UpdateMask.GetPaths(),
	})
	if err != nil {
		switch {
		...
		case errors.Is(err, user.ErrInvalidField):
			return nil, status.Errorf(codes.InvalidArgument, "invalid update field")
		default:
